...
class Image_Barcode_Code39
{
    ...
    *
    */
    function plot($noText = false, $bHeight = 0, $resolution = 300)
    {
	   ...
	   // Calculate the width of the barcode based on the resolution
	   $barcode_len = strlen($barcode);
	   //$barcode_width = ($barcode_len * $resolution) / 72; // 1 inch = 72 pixels

       /* // Create GD image object
       $img = imagecreate( $barcode_len, $this->_barcodeheight ); */
	   
	   // Create GD image object with true color
	   $img = imagecreatetruecolor($barcode_len, $this->_barcodeheight);
	   //$img = imagecreatetruecolor($barcode_width, $this->_barcodeheight);
